...
void main() {
    ...
    if (NormalOn == 1) {
      normal = mat3(tan, btan, frag_normal) * normalize(2*(texture2D(NormalMap, frag_UV).rgb) - vec3(1.0));
    }

    if(SpecularOn == 1) {
      specular_factor = texture2D(SpecularMap, frag_UV).z;
    }
